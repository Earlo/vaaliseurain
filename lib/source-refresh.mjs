...
function updateHealth(current, results, checkedAt) {
  ...
  return current.map((source) => {
    ...
    if (result.ok) {
      return {
        ...source,
        url: result.parsed?.url || source.url,
...
export async function refreshDumaSources({ fetchImpl = fetch, persist = true } = {}) {
  ...
  const results = await Promise.all(sources.map(async (source) => {
    try {
      ...
      if (source.kind === 'cec') return { id: source.id, ok: true, parsed: parseCecTelegram(html), integration: source.integration };
      if (source.kind === 'rbc') return { id: source.id, ok: true, parsed: parseRbcElectionFeed(html), integration: source.integration };
      if (source.kind === 'public-chamber') return { id: source.id, ok: true, parsed: parsePublicChamber(html), integration: source.integration };
      if (source.kind === 'metadata') return { id: source.id, ok: true, parsed: parsePageMetadata(html, source.url), integration: source.integration };
      return { id: source.id, ok: true, parsed: null, integration: source.integration };
    } catch (error) {
  ...
  if (rbc) {
    patch.electronicVoting = (patch.electronicVoting || project.electronicVoting).map((item) => {
      ...
      return {
        ...
        ...incoming,
        status: 'reported',
        statusLabel: 'RBC live map · approximate counts'
      };
    });
