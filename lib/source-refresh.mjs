...
import { refreshResultFeed } from './result-feed.mjs';
...
const youtubeBroadcastUrl = 'https://www.youtube.com/watch?v=7C0FTDxSIK4';
const youtubeBroadcastEmbedUrl = 'https://www.youtube.com/embed/7C0FTDxSIK4';
...
function migrateBroadcast(broadcast) {
  ...
  const streams = Array.isArray(broadcast.streams) && broadcast.streams.length
    ? broadcast.streams
    : [broadcast.primary, broadcast.fallback].filter(Boolean);
  ...
  const youtubeStream = {
    id: 'youtube-election-stream',
    name: 'Election night · YouTube live stream',
    label: 'Open live stream on YouTube',
    url: youtubeBroadcastUrl,
    embedUrl: youtubeBroadcastEmbedUrl,
    status: 'live',
    statusLabel: 'Live now'
  };
  ...
  return {
    ...
    primary,
    fallback,
    streams: [primary, fallback, youtubeStream]
  };
}
...
export async function refreshDumaSources({ fetchImpl = fetch, persist = true } = {}) {
  ...
  let snapshot = persist ? await updateProjectFromSources(slug, patch) : { ...project, ...patch };
  const resultFeed = await refreshResultFeed({ slug, project: snapshot, fetchImpl, persist });
  snapshot = resultFeed.snapshot;
  return {
    checkedAt,
    resultFeed: { configured: resultFeed.configured, ok: resultFeed.ok, error: resultFeed.error },
