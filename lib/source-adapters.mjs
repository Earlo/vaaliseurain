...
export function parseRbcElectionFeed(xml) {
  const electionItems = [...xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)]
    .map((match) => match[1])
    .filter((block) => /(?:Как проходят выборы в Госдуму|Как идет онлайн-голосование на выборах в Госдуму)/iu.test(xmlValue(block, 'title')));
  const item = electionItems.find((block) => /Как проходят выборы в Госдуму/iu.test(xmlValue(block, 'title')))
    ?? electionItems[0];
  if (!item) return null;
